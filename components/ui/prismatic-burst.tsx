...
// PrismaticBurst (React Bits, adaptado): shader WebGL de raymarching (via
// ogl) que desenha raios de luz girando em 3D. Colado quase verbatim do
// componente de referencia — a unica mudanca de conteudo e a paleta,
// forcada pra tons de cinza (ver `colors` em Formulario.tsx) pra caber no
// sistema monocromatico do site (nada de cor, so branco em opacidades).
//
// Ja usado antes em Pricing.tsx (deletado em 2026-08-20 quando a secao de
// Precos saiu — ver CLAUDE.md). Restaurado do historico do git (commit
// ee66711) pra reaproveitar as otimizacoes ja aplicadas — ver abaixo — em
// vez de partir do zero.
//
// ATENCAO DE PESO: isso e MUITO mais pesado que uma animacao CSS — e um
// shader fragment rodando raymarching (24 passos por pixel, ja reduzido de
// 44) no canvas inteiro, todo frame, via WebGL. Ja vem com tres cortes de
// custo: dpr 0.5 (ver comentario mais abaixo), teto de ~30fps, e pausa
// sozinho quando sai da tela (IntersectionObserver) ou a aba fica em
// background (document.hidden). Mesmo assim, em notebooks fracos/GPU
// integrada o custo pode ainda ser sensivel — raymarching e custo POR
// PIXEL, sem ajuste de parametro que zere isso. Se voltar a pesar, os
// botoes mais diretos sao: reduzir `rayCount`, reduzir `intensity`, ou
// aplicar o mesmo tratamento que a secao de Precos tinha (fallback pra um
// gradiente estatico em `useLowPowerDevice()` — ver CLAUDE.md).
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    if ("ResizeObserver" in window) {
      ...
      // Cast necessario: apos o narrowing do `in` acima, o TS estreita o tipo
      // de `window` no ramo else pra algo que nao inclui addEventListener.
  ...
  return <div className="h-full w-full relative overflow-hidden" ref={containerRef} />;
};
